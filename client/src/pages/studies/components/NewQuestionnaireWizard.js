...
export default function({cancelNewQuestionnaire, study}) {
  ...
  const [individualDetails, setIndividualDetails] = useState(study.individual_details);
  const [questionnaireDetails, setQuestionnaireDetails] = useState(study.questionnaire_details);
  ...
  async function handleSetFilteredTemplates() {
    ...
    const res = await loadTranslations({ variables: { filter: {language: language} } })
    ...
    for (const template of globalTemplates) {
        ...
        filteredTemplatesTemp.push({_id: template._id, name: template.name, language: language, inputs: inputs})
    }
    for (const template of studyTemplates) {
      if (template.language === language) {
        filteredTemplatesTemp.push(template)
      }
    }
    setFilteredTemplates(filteredTemplatesTemp)
  }

  const handleCreateNewTemplate = async (name, inputs) => {
    const newTemplate = {_id: v4(), name: name, language: getLanguage(), inputs: inputs}
		await updateStudy({variables: {id: study._id, record: {custom_templates: [...study.custom_templates, newTemplate]}}})
  ...
  const handleFinish = async () => {
    const newQuestionnaireData = {
      ...
      inputs: chosenTemplate.inputs.map(input => {return {...input, answer: {}}})
    }

    const res = await createQuestionnaire({variables: {record: newQuestionnaireData}})
    navigate("/studies/" + study._id + "/questionnaires/" + res.data.questionnaireCreateOne.recordId)
  };
      ...
      <Stepper activeStep={activeStep}>
        ...
        <Fragment>
          <Typography component={"div"} sx={{ mt: 2, mb: 1 }}>
            {activeStep === 2 ? <InputsSelection templates={filteredTemplates} chosenTemplate={chosenTemplate} setChosenTemplate={handleSetChosenTemplate} createNewTemplate={handleCreateNewTemplate}></InputsSelection> :
