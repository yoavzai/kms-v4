...
export default function({templates, chosenTemplate, setChosenTemplate, createNewTemplate}) {
    ...
    return (
        <div>
            ...
            <RadioGroup
                ...
                onChange={(e) => setChosenTemplate(templates.find(t => t._id === e.target.value))}
            >
            {templates.map((t) => <FormControlLabel checked={t._id === chosenTemplate._id} key={t._id} value={t._id} control={<Radio />} label={t.name} />)}
            </RadioGroup>
            </FormControl>
            <h3>Inputs</h3>
            <List>
            ...
            {isNewTemplate &&
                <Dialog open={true}>
                <DialogContent>
                    <NewTemplateWizard templates={templates} createNewTemplate={handleCreateNewTemplate} cancelNewTemplate={handleCancelNewTemplate}></NewTemplateWizard>
                </DialogContent>
                </Dialog>
            }
        </div>
