...
export default function ({
  ...
  finishEditQuestionnaire,
}) {
  const languageField = questionnaire.questionnaire_details.find(
    (field) => field.key === "language"
  );
  const [activeStep, setActiveStep] = useState(0);
  const [individualDetails, setIndividualDetails] = useState(
    questionnaire.individual_details
  );
  const [questionnaireDetails, setQuestionnaireDetails] = useState(
    questionnaire.questionnaire_details.filter(
      (field) => field.key != "language"
    )
  );
  ...
  const handleFinish = async () => {
    const newQuestionnaireData = {
      ...
      questionnaire_details: [languageField, ...questionnaireDetails],
    };
  ...
  function Summary() {
    return (
      <div>
        ...
        {[languageField, ...questionnaireDetails].map((field) => {
          return (
            <div key={field.key}>
              <span>
                {field.key} - {field.value}
    ...
    <Box sx={{ width: "100%" }}>
      ...
      <Fragment>
        ...
        <Box sx={{ display: "flex", flexDirection: "row", pt: 2 }}>
          ...
          <Button onClick={() => cancelEditQuestionnaire()}>Cancel</Button>
          <Box sx={{ flex: "1 1 auto" }} />
          {activeStep < steps.length - 1 ? (
            <Button onClick={() => setActiveStep((prevActiveStep) => prevActiveStep + 1)}>Next</Button>
          ) : (
